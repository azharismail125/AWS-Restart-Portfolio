...
![Lab Architecture Overview](Instance%20Overview%201.png)
...
![EC2 Instance Dashboard](Instance%20Initiailized%202.png)
...
![Serial Console Boot Output](Instance%20Check%203.jpg)
...
![HTTP Access Verification](Instance%20Security%20Group%20update%204.png)
...
To accommodate growing storage and throughput demands, I scaled the backing Amazon Elastic Block Store (EBS) volume (`vol-0561d7e0c02537c2f`). I dynamically altered the storage configuration to a `gp3` type with a `10 GiB` capacity, provisioning 3000 IOPS and 125 MB/s throughput on-the-fly without requiring an instance reboot.

![EBS Volume Modification](Instance%20EBS%20Volume%20Modified%205.png)
...
To validate the resilience of my deployment against human error, I attempted to terminate the instance directly from the console actions menu while protection features were active. The platform successfully blocked the execution block.

![Termination Blocked Error](Instance%20Termination%20in%20Action%206.png)
...
Once testing was complete and the infrastructure lifecycle ended, I manually updated the instance attributes to remove the termination lock. With the guardrail deleted, I successfully executed a clean, permanent deletion of the resource.

![Successful Deletion](Instance%20Terminated%207.png)
