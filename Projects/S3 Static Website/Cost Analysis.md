...
Of the three, data transfer out is almost always the dominant cost for a live site — storage and requests stay cheap even at scale, but as each visitor downloads the page's files, data transfer out increases rapidly.

![Billing and Cost Management dashboard](Projects/S3%20Static%20Website/S3%20Screenshots/5_Billing_and_Cost_Management_Dashboard.png)
...
For a small local gym's marketing site with modest traffic (roughly a few thousand visitors per month) and optimized images, the realistic cost is under R100/month on S3 alone. Traffic combined with image weight is the number that moves the most — a business expecting growth or planning promotions should budget for data transfer scaling.
